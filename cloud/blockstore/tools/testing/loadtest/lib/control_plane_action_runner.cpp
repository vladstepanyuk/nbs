    ...
    const NProto::TActionGraph::TControlPlaneAction& action)
{
    ...
    for (auto& coordinate: request.GetDevicesToReplace()) {
        ...
        NProto::TReplaceDeviceRequest replaceDeviceRequest;
        replaceDeviceRequest.SetDiskId(diskId);
        replaceDeviceRequest.SetDeviceUUID(deviceUUID);

        TStringStream input;
        replaceDeviceRequest.PrintJSON(input);
