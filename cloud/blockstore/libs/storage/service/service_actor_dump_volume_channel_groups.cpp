...
class TDumpVolumeChannelGroupsActor final
    : public TActorBootstrapped<TDumpVolumeChannelGroupsActor>
{
private:
...
TResultOrError<IActorPtr> TServiceActor::CreateDumpVolumeChannelGroupsActor(
    TRequestInfoPtr requestInfo,
    TString input)
{
    ...
    TVector<TString> replicatedDiskIds;

    for (const auto& [diskId, volumeInfo]: State.GetVolumes()) {
        if (volumeInfo->IsDiskRegistryVolume()) {
            replicatedDiskIds.emplace_back(diskId);
        }
    }
